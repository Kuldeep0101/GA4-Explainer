...
export default function ClientReport({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    // Fetch user plan for white-labeling
        ...
        <div id="report-content" className={styles.reportContainer}>
          ...
          <div className={styles.reportHeader}>
            <div>
              ...
              {reportData.report.report_date_range && reportData.report.report_date_range.toLowerCase() !== 'not specified' && (
            ...
            <div className={styles.highlightsGrid}>
              {reportData.report.analysis_sections.map((h, i) => (
                <div key={i} className={`${styles.highlight} ${sentimentClass(h.sentiment)}`}>
                  ...
                  {/* visual_tip intentionally hidden from PDF output */}
                </div>
              ))}
            </div>
          </div>
          ...
          <div className={styles.reportFooter}>
            {userPlan !== 'agency' ? (
              <span>
                Generated by{' '}
                <a href="https://ga4explainer.online/" target="_blank" rel="noreferrer" style={{ color: 'inherit', textDecoration: 'underline' }}>GA4 Explainer</a>
              </span>
            ) : <span />}
            <span>
              Report generated on &mdash;{' '}
